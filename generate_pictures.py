...
def autoHTML (name, link, alt, templatePath, pageURL):
    foFILE_sheet = open (templatePath+"/sheet-pictures-"+name+".html", "w")
...
def createPic(imgList, templatePath, pagesList, imagePerPage, imagePerCol, pagesHtml):
    ...
    for deviation in imgList :
        ...
        alt = format(deviation.title)+" photo deviantart sinason"
        name = pageURL
        name = re.sub(r'(.*)?\/',"", name)
        if ((currentIMG%imagePerPage)==0):
            if (currentIMG != 0):
                ...
                borderCount +=1
                if (borderCount >= pagesHtml):
                    borderCount = pagesHtml-1
                    break 
        ...
        pagesList[borderCount]["foFILE"].write("\t\t\t<a href=\"/sheet-pictures-"+str(name)+"\" target=\"_blank\" class=\"gallery__link\">\n")
    ...
    for pagesFiles in range(pagesHtml):
        pagesList[pagesFiles]["foFILE"].write("\n")
        pagesList[pagesFiles]["foFILE"].write("    <p>Pages: \n")
        for pagesFilesWrite in range(pagesHtml):
            pagesList[pagesFiles]["foFILE"].write("    <a href=\"/"+pagesList[pagesFilesWrite]["filename"]+"\" class=\"buttonPage\">"+str(pagesFilesWrite+1)+"</a>\n")
        pagesList[pagesFiles]["foFILE"].write("    </p>\n")
...
def gene_pic_gallery():
    ...
    da = deviantart.Api("16837", "2d0910b37a821559567970f10a583733")
    ...
    print ("DEVIANTART SINASON: total deviations : ",total)
    
    imagePerPage = 24
    imagePerCol = int(imagePerPage/4)
    pagesHtml = 8 # round((total/imagePerPage)+0.5)
    ...
    for line in foFILEout:
        ...
        if (re.search("start autogenerated", line)):
            copy = 0    
            createPic(deviations, templatePath, pagesList, imagePerPage, imagePerCol, pagesHtml)
