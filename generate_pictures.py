...
def createPic(imgList, templatePath, pagesList, imagePerPage):
    for pages in range(len(imgList)):
        ...
        for imageNb in range(totalhits) :
            currentIMG = imageNb+pages*200 # PIXABAY PAGE contains 200 pictures
...
def createPicInsect(imgList, foFILE, templatePath):
    currentIMG = 0
    for pages in range(len(imgList)):
        imageData = imgList[pages]
        totalhits = imageData['totalHits']
        ...
        for imageNb in range(totalhits) :
            ...
            if (re.search("insect|dragonfly|butterfly|mantis|ladybug|bee|chafer|caterpillar", concate)):
                foFILE.write("     <input class=\"gallery__select\"  type=\"radio\" name=\"autogenerated\" id=\""+str(currentIMGint)+"\"/>\n")
                currentIMGint +=1
        ...
        for imageNb in range(totalhits) :
            ...
            if (re.search("insect|dragonfly|butterfly|mantis|ladybug|bee|chafer|caterpillar", concate)):
                ...
                name = name.replace("_960_720.jpg","")
                name = re.sub(r'(.*)?\/', '', name)
                foFILE.write("         <label class=\"gallery__item\" for=\""+str(currentIMGint)+"\">\n")
                ...
                foFILE.write("         </label>\n")
                currentIMGint +=1
                #### Do not generate file HTML for picture as it is already exist 
        currentIMG = currentIMGint
        
    foFILE.write("    </div> <!-- End div classe gallery -->\n")
    return currentIMG
    
def createPicGiverny(imgList, foFILE, templatePath):
    currentIMG = 0
    for pages in range(len(imgList)):
        imageData = imgList[pages]
        totalhits = imageData['totalHits']
        ...
        for imageNb in range(totalhits) :
            alt = imageData['hits'][imageNb]['tags']
            concate = alt+" "+imageData['hits'][imageNb]['pageURL']
            if (re.search("giverny|monet|nympheas", concate)):
                foFILE.write("     <input class=\"gallery__select\"  type=\"radio\" name=\"autogenerated\" id=\""+str(currentIMGint)+"\"/>\n")
                currentIMGint +=1
...
def createPicMoutain(imgList, foFILE, templatePath):
    currentIMG = 0
    for pages in range(len(imgList)):
        imageData = imgList[pages]
        totalhits = imageData['totalHits']
        ...
        for imageNb in range(totalhits) :
            alt = imageData['hits'][imageNb]['tags']
            concate = alt+" "+imageData['hits'][imageNb]['pageURL']
            if (re.search("mountain|alps", concate)):
                foFILE.write("     <input class=\"gallery__select\"  type=\"radio\" name=\"autogenerated\" id=\""+str(currentIMGint)+"\"/>\n")
                currentIMGint +=1
...
def createPicFlower(imgList, foFILE, templatePath):
    currentIMG = 0
    for pages in range(len(imgList)):
        imageData = imgList[pages]
        totalhits = imageData['totalHits']
        ...
        for imageNb in range(totalhits) :
            alt = imageData['hits'][imageNb]['tags']
            concate = alt+" "+imageData['hits'][imageNb]['pageURL']
            if ((re.search("flower|blossom|garden|rose", concate)) and (not re.search("chafer|bee|insect",concate))):
                foFILE.write("     <input class=\"gallery__select\"  type=\"radio\" name=\"autogenerated\" id=\""+str(currentIMGint)+"\"/>\n")
                currentIMGint +=1                
...
def createPicLiving(imgList, foFILE, templatePath):
    currentIMG = 0
    for pages in range(len(imgList)):
        imageData = imgList[pages]
        totalhits = imageData['totalHits']
        ...
        for imageNb in range(totalhits) :
            alt = imageData['hits'][imageNb]['tags']
            concate = alt+" "+imageData['hits'][imageNb]['pageURL']
            if (re.search("insect|dragonfly|butterfly|mantis|ladybug|bee|chafer|caterpillar|animal|bird|cat|frog|reptile|snake", concate)):
                foFILE.write("     <input class=\"gallery__select\"  type=\"radio\" name=\"autogenerated\" id=\""+str(currentIMGint)+"\"/>\n")
                currentIMGint +=1                
...
def gene_pic_gallery():
    ...
    foFILEInsect = open (templatePath+"/htmlfileInsects.txt", "w");
    foFILELiving = open (templatePath+"/htmlfileLiving.txt", "w");
    foFILEGiverny = open (templatePath+"/htmlfileGiverny.txt", "w");
    foFILEMountain = open (templatePath+"/htmlfileMountain.txt", "w");
    foFILEFlower = open (templatePath+"/htmlfileFlower.txt", "w");
    copy = 1
    for line in foFILEout:
        if (copy == 1):
            foFILEInsect.write (line)
            foFILELiving.write (line)
            foFILEGiverny.write (line)
            foFILEMountain.write (line)
            foFILEFlower.write (line)
        ...
        if (re.search("start autogenerated", line)):
            copy = 0    
            imgPicsAll = createPic(imgList, templatePath, pagesList, imagePerPage)
            imgPicInsect = createPicInsect(imgList, foFILEInsect, templatePath)
            imgPicLiving = createPicLiving(imgList, foFILELiving, templatePath)
            imgPicGiverny = createPicGiverny(imgList, foFILEGiverny, templatePath)
            imgPicMountain = createPicMoutain(imgList, foFILEMountain, templatePath)
            imgPicFlower = createPicFlower(imgList, foFILEFlower, templatePath)
      
    for pagesFiles in range(0, pagesHtml):
        os.system("mv "+templatePath+"/"+pagesList[pagesFiles]["htmlfile"]+".txt "+templatePath+"/"+pagesList[pagesFiles]["filename"]+".html")
        
    os.system("mv "+templatePath+"/htmlfileInsects.txt "+templatePath+"/galerie_pictures-insects.html")
    os.system("mv "+templatePath+"/htmlfileLiving.txt "+templatePath+"/galerie_pictures-living.html")
    os.system("mv "+templatePath+"/htmlfileGiverny.txt "+templatePath+"/galerie_pictures-giverny.html")
    os.system("mv "+templatePath+"/htmlfileMountain.txt "+templatePath+"/galerie_pictures-mountain.html")
    os.system("mv "+templatePath+"/htmlfileFlower.txt "+templatePath+"/galerie_pictures-flower.html")
    ...
    maximumPicPerPage = max(imgPicsAll, imgPicFlower, imgPicGiverny, imgPicInsect, imgPicLiving, imgPicMountain)
    ...
    foFILE.write("    grid-template-rows: repeat("+str(round(maximumPicPerPage/4))+", 300px);\n")
    foFILE.write("}\n")
    foFILE.write("\n")
    ...
    for nbr in range(maximumPicPerPage):
